...
def get_imp_price(token: str) -> float:
    if token == 'imp':
        ...
        if not web3.is_connected():
            raise ConnectionError("Failed to connect to the Fantom node")
        ...
        if contract:
            try:
                ...
                impBalanceCall = contract.functions.getPoolTokenInfo(
                    ...
                    Web3.to_checksum_address('0x1C1d786f88eaf8fD1E0939757520beC1413310FE')).call()
                impBalance = impBalanceCall[0]

                # balance of MIM
                mimBalanceCall = contract.functions.getPoolTokenInfo(
                    '0x8c3c964c2d08679d3d09866cf62c5b14a5346479000100000000000000000207',
                    Web3.to_checksum_address('0x82f0B8B456c1A451378467398982d4834b6829c1')).call()
                mimBalance = mimBalanceCall[0]

                # balance of DAI
                daiBalanceCall = contract.functions.getPoolTokenInfo(
                    '0x8c3c964c2d08679d3d09866cf62c5b14a5346479000100000000000000000207',
                    Web3.to_checksum_address('0x8D11eC38a3EB5E956B052f67Da8Bdc9bef8Abf3E')).call()
                daiBalance = daiBalanceCall[0]

                # balance of WFTM
                ftmBalanceCall = contract.functions.getPoolTokenInfo(
                    '0x8c3c964c2d08679d3d09866cf62c5b14a5346479000100000000000000000207',
                    Web3.to_checksum_address('0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83')).call()
